...
import { isNoAuthPage } from 'src/utils';
import { IS_SUB } from 'src/utils/config';
...
export default {
    ...
    async getMenuData() {
        ...
        const role = getLoginUser().roleId


        if (role === 3) {
            return [
                {id: 1, title: '系统管理', order: 900, type: 1},
                {id: 2, parentId: 1, title: '患者管理', path: '/patient', order: 900, type: 1},
            ];
        } else if (role === 2) {
            return [
                {id: 1, title: '系统管理', order: 900, type: 1},
                {id: 2, parentId: 1, title: '用户管理', path: '/users', order: 900, type: 1},
                {id: 3, parentId: 1, title: '排班管理', path: '/schedule', order: 900, type: 1},
                {id: 4, parentId: 1, title: '科室管理', path: '/department', order: 900, type: 1},
            ];
        }
